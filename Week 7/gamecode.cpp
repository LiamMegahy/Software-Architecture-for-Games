...
ErrorType Game::StartOfGame()
{
	...
	//Setting the new ships spawn location within a Vector2D
	Spaceship* pShip2;
	//Creating a new pointer to a spaceship, calling it pShip2
	pShip2 = new Spaceship(ObjectManager);
	//Setting pShip2 to be a new Spaceship, of type ObjectManager - this allows the object manager to be able to work with and handle this new ship
	pShip2->initialise(location);
	//Setting the ships initial location using the Vector2D variable on line 280
	ObjectManager.AddObject(pShip2);
	//Adding this new ship to the ObjectManagers list of game objects so it can handle the updating and rendering of the ship

	for (int i = 0; i < 8; i++) {
		Rock* pRock;
		//Creating a new pointer to a rock, calling it pRock - similar to the new spaceship
		pRock = new Rock();
		//Creating a new instance of Rock
		pRock->Initialise(Vector2D(rand()%2000 - 1000, rand() % 2000 - 1000), rand()% 628/100.0);
		//Setting the initialise location to be of a 2D Vector with 2 random numbers for each value (within the limits of the screens pixle count)
		ObjectManager.AddObject(pRock);
		//Passing the new rock to the object manager to handle
		//This entire process will repeat 8 times to create 8 different rocks at different locations on the screen
	}
	gt.mark();
	//Setting frametime
	return SUCCESS;
...
ErrorType Game::Update()
{
	...
	gt.mark();
	//Setting frame time
	ObjectManager.runFunctions();
	//This function runs the functions "RenderAll", "UpdateAll" and "DeleteInactive" within ObjectManager.cpp
	//Simplifies the code needed within Gamecode.cpp to just one function call 
	// *********************************************************************
	// *********************************************************************
...
// called when the game ends by returning to main menu
{
	...
	ObjectManager.deleteThis();
	//At the end of the game, the "DeleteThis" function within ObjectManager will take any and all GameObjects stored within its list and delete both the pointer and the contents of the pointer
	return SUCCESS;
}
