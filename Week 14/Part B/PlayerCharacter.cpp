...
void PlayerCharacter::move() {
	...
	if (pInputs->KeyPressed(DIK_W)) {
		acceleration.setBearing(angle, (500.0f * speedModifier));
	}
	if (pInputs->KeyPressed(DIK_S)) {
		acceleration.setBearing(angle, (-50.0f * speedModifier));
	}
	...
	if (pInputs->KeyPressed(DIK_5)) {
		if (hasSpeedModifier != true) {
			if (score >= 150) {
				speedModifier = speedModifier + 0.15;
				score = score - 150;
				GameObject::loadSound(L"target1.wav");
