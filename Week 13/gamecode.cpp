...
ErrorType Game::StartOfGame()
{
	...
	pPlayerCharacter2->initialise(location, &ObjectManager, &lm);
	lm.initailise(&ObjectManager);
	ObjectManager.AddObject(pPlayerCharacter2);
	for (int i = 0; i < 1; i++) {
		Rock* pRock;
		pRock = new Rock;
		pRock->Initialise(Vector2D(rand() % 2000 - 1000, rand() % 2000 - 1000), rand() % 628 / 100.0, &ObjectManager, &lm);
...
ErrorType Game::Update()
{
	...
	gt.mark();
	ObjectManager.runAllFunctions();
	lm.update(gt.mdFrameTime);
	lm.render();
