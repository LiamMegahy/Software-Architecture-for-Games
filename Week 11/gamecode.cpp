...
ErrorType Game::StartOfGame()
{
	...
	pPlayerCharacter2->initialise(location, &ObjectManager);
	ObjectManager.AddObject(pPlayerCharacter2);
	for (int i = 0; i < 8; i++) {
		Rock* pRock;
		pRock = new Rock;
		pRock->Initialise(Vector2D(rand()%2000 - 1000, rand() % 2000 - 1000), rand()% 628/100.0, &ObjectManager);
		ObjectManager.AddObject(pRock);
		ObjectManager.addEnemy(pRock);
	}
